...
static void scan_dma_engines(struct c985_poc *d)
{
    ...
    dev_info(&d->pdev->dev, "=== SCANNING DMA ENGINES ===\n");
    ...
    dev_info(&d->pdev->dev, "Found %d S2C (write) and %d C2S (read) engines\n",
             s2c_count, c2s_count);

    if (s2c_count > 0 && c2s_count > 0) {
        engines_found = true;
        dev_info(&d->pdev->dev, "Using: Write=DMA[%d]@0x%x, Read=DMA[%d]@0x%x\n",
                           ...
                           bool is_write)
{
    ...
    dev_info(&d->pdev->dev, "=== DMA %s: %zu bytes, card=0x%08x ===\n",
             name, size, card_addr);
    ...
    dev_info(&d->pdev->dev, "Need %d descriptors (%d data + %d dummy)\n",
    ...
    dev_info(&d->pdev->dev, "Descriptors: phys=0x%llx aligned=0x%llx\n",
    ...
    if (is_write) {
        ...
        dev_info(&d->pdev->dev, "  Dummy[%d]: ctrl=0x%08x len=4 card=0x%llx\n",
    ...
    /* Verify write */
    {
        ...
        dev_info(&d->pdev->dev, "Descriptor ptr set: 0x%08x%08x\n", hi, lo);
    }

    /* Start transfer */
    dev_info(&d->pdev->dev, "Starting %s DMA...\n", name);
    writel(DMA_CTRL_START, d->bar0 + engine->base + DMA_REG_CTRL_STATUS);

    /* Immediately check status */
    status = readl(d->bar0 + engine->base + DMA_REG_CTRL_STATUS);
    dev_info(&d->pdev->dev, "Status after start: 0x%08x\n", status);
                        ...
                        u32 card_addr, size_t size)
{
    ...
    dev_info(&d->pdev->dev, "Write buffer: virt=%p phys=0x%llx size=%zu\n",
                       ...
                       u32 card_addr, size_t size)
{
    ...
    dev_info(&d->pdev->dev, "Read buffer: virt=%p phys=0x%llx size=%zu\n",
             buf, (u64)buf_phys, size);
    ...
    if (ret == 0) {
        ...
        /* Dump first 64 bytes */
        dev_info(&d->pdev->dev, "Read data (first 64 bytes):\n");
